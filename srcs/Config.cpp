...
void Config::parseLocationBlock(std::ifstream &file, Server &server, const std::string &openLine)
{
	...
	{
		std::cerr << "Error: Location path must be an absolute path starting with '/': " << path << std::endl;
		exit(1);
	}
	if (extra != "{")
	{
		std::cerr << "Error: Invalid syntax in location directive, expected '{' after path: " << line << std::endl;
		exit(1);
	}
